...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, DDEml,MemStructsU, Grids,ConstDef;
...
TItemDDEs = class(TList)
  ...
  public
   ...
   procedure addItem(name: string; server: string; topic: string; num: integer; connect: boolean;_typ: integer);
...
function getvalbvy(val: single; typ: integer): string;
begin
   case typ of
    TYPE_DISCRET: begin
       if val=0 then result:='0' else result:='1';
    end;
    TYPE_INTEGER: result:=inttostr(round(val));
    else result:=floattostr(val);
   end;
end;
...
procedure TServInfos.addItem(name: string; server: string; topic: string; num: integer; connect: boolean; _typ: integer);
...
begin
   ...
   itlist.addItem(name,server,topic,num,connect,_typ);
end;
...
procedure TItemDDEs.addItem(name: string; server: string; topic: string; num: integer; connect: boolean;_typ: integer);
var FHszItem: HSZ;
    ddeRslt: LongInt;
    hdata: HDDEData;
    it: PTItemDDE;
...
begin
    ...
    for j:=0 to items[id].count-1 do
                   begin
                      fThrtItems.SetVal(items[id].rtid[j],val,100);
               ...
               begin
                 ...
                 if (trim(dataval)='') then
                   begin
                      items[i].err:=3;
                       for j:=0 to items[i].count-1 do
                      fThrtItems.ValidOff(items[i].rtid[j]);
                      exit;
                   end;
                           ...
                           begin
                              if (MinRaw=MaxRaw) {or (MinEU=MaxEU)} then
                                 ...
                                 //  fThrtitems.SetValid(rtid,100);
                                   fThrtitems.SetVal(rtid,val,100);
                                 end
                              else
                                 begin
                                   if (MinRaw<MaxRaw) then
                                       begin
                                        ...
                                        // fThrtitems.SetValid(rtid,100);
                                         fThrtitems.SetVal(rtid,val_t,100);
                                        end
                                    else
                                        begin
...
begin
 ...
 ids:=id;
 try
 temp:=trim(getvalbvy(val,items[ids].typ_));
 Data:=Pchar(temp);
 except
// showmessage('');
 end;
 ...
 //   0, items[ids].servHandle, cf_Text, 0);
 // if hszDat <> 0 then
  begin

    hdata := DdeClientTransaction(Data, StrLen(Data)+1, conv, items[ids].servHandle,
      CF_TEXT, XTYP_POKE, TIMEOUT_ASYNC, nil);
    Result := hdata <> 0;
  end // else
     //begin
     //   err:=DdeGetLastError(DdeInstId);
    //    err:=DdeGetLastError(DdeInstId);
    // end;
  end;
                ...
                begin
                  ...
                  ddit:=fThrtItems.GetDDEItem(i);
                     if ((ddit<>'') {and (iscorrect(ddit))}) then
                         serInf.addItem(ddit,app,topic,i,(fThrtItems.Items[i].refCount>0),fThrtItems.Items[i].logTime);
                end;
           end;
      end;
//  itemlist.Sort;
...
begin
      ...
      begin
        ids:=fThrtItems.Command[j].ID;
        itli := self.FindListByrtId(fThrtItems.Command[j].ID,cN);
        if itli<>nil then
        begin
        newComValue := getconvcomm(fThrtItems.Command[j].ValReal,
         fThrtItems[ids].MinEU,fThrtItems[ids].MaxEU,fThrtItems[ids].MinRaw,fThrtItems[ids].MaxRaw);
        if cN <> -1 then
        begin
       ...
       begin
         ...
         if (itli.items[k].activ>0) then
           begin
              ...
              begin
              fThrtItems.ValidOff(itli.items[k].rtid[0]);
