...
type
  ...
  public
    ...
    procedure RegisterPropertyHelperM(ReadFunc,
...
function ClassCallProc31(Caller: TIFPSExec; p: PIFProcRec; Global, Stack: TIfList): Boolean;
var
  I, ParamCount: Longint;
  Params: TIfList;
  n: PIfVariant;
...
begin
  ...
  begin
    ...
    InnerfuseCall(Caller, FSelf, p^.Ext1, ccRegister, Params, n, @ResourcePtrSupport);
    Params.Free;
    Result := True;
  end else begin
    n := Stack.GetItem(Stack.Count -1);
...
procedure TIFPSRuntimeClass.RegisterPropertyHelperM(ReadFunc,
  WriteFunc: Pointer; const Name: string);
var
  P: PClassItem;
begin
  ...
  //p^.FNameHash := MakeHash(Name);
  p^.b := 2;
 // p^.FReadFunc := ReadFunc and $00FFFFFF;
 // p^.FWriteFunc := WriteFunc and $00FFFFFF;
  FClassItems.Add(p);
end;
