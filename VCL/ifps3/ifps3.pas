...
interface
uses
  SysUtils, ifps3utl, ifps3common, Dialogs;
...
begin
  if FStatus <> isNotLoaded then begin
    ...
    FStatus := isPaused;
    Result := RunScript;
    if ExceptionCode <> erNoError then
        sHOWmESSAGE('[Runtime Error] : ' + TIFErrorToString(ExceptionCode, ExceptionString) +
          ' in ' + IntToStr(ExceptionProcNo) + ' at ' + IntToSTr(ExceptionPos));
