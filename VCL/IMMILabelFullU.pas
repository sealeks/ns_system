...
uses controls, classes, stdctrls, messages, sysUtils, Forms, Graphics,
     constDef, Expr, IMMILabelEditorU, dialogs, IMMIPropertysU, windows;

type
   TWinCont = class(TWinControl);

type
  TboundsString = class (tPersistent)
  private

    fAccess: integer;
  public
     fNAG, fNPG, fVPG, fVAG: TExprStr;
     fNAGex, fNPGex, fVPGex, fVAGex: tExpretion;
  published
    property NAG: TExprStr read fNAG write fNAG;
    property NPG: TExprStr read fNPG write fNPG;
    property VPG: TExprStr read fVPG write fVPG;
    property VAG: TExprStr read fVAG write fVAG;
    property iAccess: integer read fAccess write fAccess;
  end;

  TIMMILabelFull = class(TLabel)
  private
...
type TimmiLabelOperator=class(Tlabel)
     private
     procedure ImmiChangeUser(var Msg: TMessage); message WM_IMMIUSERCHANGED;
     procedure ImmiInit (var Msg: TMessage); message WM_IMMIINIT;
     procedure ImmiUpdate (var Msg: TMessage); message WM_IMMIUPDATE;
     end;
...
Uses IMMIAnalogPropU, MemStructsU, AnalogpropGraphic;

procedure TIMMILabelFull.ShowPropForm;
begin
  with TLabelEditorFrm.Create(nil) do begin
...
procedure TIMMILabelFull.ShowPropFormMsg(var Msg: TMessage);
begin
  IMMIUninit(Msg);
...
procedure TIMMILabelFull.Click;
var
  id: longInt;
  i,j: integer;
  alg: TAnalogGraph;
    ...
    if not fisgraphic then
      begin
       ...
       with Alg do
          ...
          //  hide;
           Immiscope1.autosize:=self.AutoSize;
...
constructor TIMMILabelFull.Create(AOwner: TComponent);
begin
...
destructor TIMMILabelFull.Destroy;
begin
     fFPosition.Free;
     fExprValue.free;
     fExprColor.free;
...
procedure TIMMILabelFull.ImmiInit (var Msg: TMessage);
begin
...
procedure TIMMILabelFull.ImmiUnInit (var Msg: TMessage);
begin
...
procedure TIMMILabelFull.Loaded;
begin
   inherited;
   initcolor:=font.color;
end;

procedure TIMMILabelFull.ImmiUpdate;
var
   val: real;
   str: string;
   newCap: string;
...
begin
  ...
  begin
   ...
   if (newCap <> Caption) and (Visible) then
   begin
     caption := newCap;
...
procedure TIMMILabelFull.ImmiBlinkOn (var Msg: TMessage);
begin
...
procedure TIMMILabelFull.ImmiBlinkOff (var Msg: TMessage);
begin
   if assigned(fExprColor) then exit;
     if assigned(fExprBlink) then
       begin
 ...
 procedure TimmiLabelOperator.ImmiChangeUser(var Msg: TMessage);
 begin
   ...
   text:=string(OperatorName^);
 end;

 procedure TimmiLabelOperator.ImmiInit (var Msg: TMessage);
  begin
   if OperatorName=nil then
     begin
       text:='???';
 ...
 procedure TimmiLabelOperator.ImmiUpdate (var Msg: TMessage);
   begin
   if OperatorName=nil then
     begin
       text:='???';
      ...
      if string(OperatorName^)<>text then text:=string(OperatorName^);
   end;

//initialization
   // if AnalogGraph=nil then AnalogGraph:=TAnalogGraph.Create(application);
