...
begin
  ...
  begin
    ...
    if ((tempnum > -1) or (uppercase(temp) = 'ETH') or (uppercase(temp) = 'TMOD')) then
    begin
      ComSet.trd := tempnum;
      ComSet.flCtrl := True;
    end;
...
var
  i, j, slave, k, comN, Bound, idxThr: integer;
  temComNum, TempEthNum, TempRModNum, TempTModNum : integer;
  serv, group, configinfo, host, group1, app, topic: string;
  ComSet: TComSEt;
  ServerThread: TServerThread;
  trn, trn1, trnopcbush, trnddebush: TtReeNode;
...
begin
  ...
  begin
    ...
    begin
      ...
      TempRModNum := isCOM(topic, 'RMOD', ComSet);
      TempTModNum := isCOM(topic, 'TMOD', ComSet);
      if ((temComNum > 0) or (tempEthNum > 0) or (TempRModNum > 0) or (TempTModNum > 0))
      and (slave > 0) then
      begin
        if (temComNum > 0) then
        begin
        ...
        if (tempEthNum > 0) then
        begin
          isCOM(topic, 'ETH', ComSet);
          idxThr := threadCOMList.IndexOf('ETH' + IntToStr(slave));
        end
        else
        if (TempRModNum > 0) then
        begin
          isCOM(topic, 'RMOD', ComSet);
          idxThr := threadCOMList.IndexOf('COM' + IntToStr(slave));
          modbus := true;
          temComNum := TempRModNum;
        end
        else
        if (TempTModNum > 0) then
        begin
          isCOM(topic, 'TMOD', ComSet);
          idxThr := threadCOMList.IndexOf('TMOD' + IntToStr(slave));
          modbus := true;
          tempEthNum := TempTModNum;
        end;
        ...
        begin
          ...
          for k := 0 to TreeView1.Items.Count - 1 do
            ...
            begin
              ...
              trn.SelectedIndex := 3;
              trn1 :=
                TreeView1.Items.AddChild(trn, group + '[' + IntToStr(slave) + ']');
              trn1.ImageIndex := 3;
              trn1.StateIndex := 3;
        ...
        begin
          ...
          begin
            ...
            ServerThread := TServerThread.Create(PathMem, temComNum, ComSet, modbus);
            threadCOMList.AddObject('COM' + IntToStr(temComNum), ServerThread);
          end
          else
          begin
            trn := TreeView1.Items.AddChild(trnopcbush, 'Eth' + IntToStr(slave));
            ServerThread := TServerThread.Create(PathMem, ComSet, slave, modbus);
  ...
  begin
    ...
    begin
      ...
      if (asc.ProterrR = 0) and (asc.ProterrW = 0) then
        ColorStringGrid2.fRows.Items[j + 1].font.color := clgreen
      else
...
begin
  ...
  begin
    ...
    begin
      ...
      if asc[j].activ then
        ColorStringGrid1.Cells[12, j + 1] := 'A'
      else
