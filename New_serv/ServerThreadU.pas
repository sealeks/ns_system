...
begin
  ...
  if pos('V', item)=0 then exit;           //добавлено СА 29.4
      ...
      begin
        ...
        begin
          ...
          PWord(str)^ := commandValue;
           ...
           numit:=frtitems.TegGroups.Idx[frtitems.TegGroups.ItemNameByNum[ascit.GroupNum]];                   // добавлено СА 29.04
                         ...
                         ctNone, ctB, ctD, ctBD, ctR:
                          ...
                          if item.Name[1] <> '$' then
                           ...
                           askItems_.Insert(item);
                             ...
                             log('Тег tg='+frtItems.GetName(j)+' не был добавлен как архивный. Неверно определен источник. Номер бита выше допустимого. sc='+frtItems.GetDDEItem(j), _DEBUG_ERROR);
                           end else
                             log('Тег tg='+frtItems.GetName(j)+' не был добавлен как архивный. Неверно определен источник sc='+frtItems.GetDDEItem(j), _DEBUG_ERROR);
                           end;
                          ctArch:
                           begin
                              ...
                              if (typ_=REPORTTYPE_HOUR) then
                                begin
                                  ...
                                  log('Тег tg='+frtItems.GetName(j)+' был добавлен как архивный. sc='+frtItems.GetDDEItem(j));
                                end else
                                  log('Тег tg='+frtItems.GetName(j)+' не был добавлен как архивный. Неверно задан тип', _DEBUG_ERROR);
                              end;
                           end;
                          ctErr:
                           begin
                              log('Тег tg='+frtItems.GetName(j)+' не был добавлен!!! Неверный источник sc='+frtItems.GetDDEItem(j),_DEBUG_ERROR);
                           end;
                         end;
                         end else
                           begin
                              log('Тег tg='+frtItems.GetName(j)+' не был добавлен!!! Неверный источник или адрес sc='+frtItems.GetDDEItem(j),_DEBUG_ERROR);
                           end;
                  except
                  log('Тег tg='+frtItems.GetName(j)+' не был добавлен!!! Неверный источник или адрес sc='+frtItems.GetDDEItem(j),_DEBUG_ERROR);
                  end;
