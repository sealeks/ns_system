...
type
  TDataPostgree = class(TMainDataModule)
...
TrendConnection.Protocol:='postgresql-8';

end;


procedure TDataPostgree.uninit();
begin
...
function TDataPostgree.Connect(): boolean;
begin
...
function TDataPostgree.DisConnect(): boolean;
begin
  if TrendConnection.Connected then  TrendConnection.Disconnect;
  result:= not TrendConnection.Connected;
end;

function TDataPostgree.getConnected(): boolean;
begin
   result:= TrendConnection.Connected;

end;

function TDataPostgree.TableExist(iFileName: String): boolean;
var
...
begin
    // ShowMessage('eeee');
     ...
     try
     TrendConnection.GetTableNames(iFileName,TableList);
...
function TDataPostgree.CreateTrTable(iFileName: String): boolean;
 var fn, s : string;
begin
  fn := iFileName;
  try
...
function TDataPostgree.CreateAlTable(iFileName: String): boolean;
 var fn, s : string;
begin
  fn := iFileName;
  try
...
function TDataPostgree.CreateRepTable(iFileName: String): boolean;
 var fn,s : string;
begin
  fn := iFileName;
  try
...
function TDataPostgree.createTrenddef(): boolean;
 var fn,s : string;
begin
     ...
     begin
       try
         with TrendQuery do
            ...
            // создать таблицу и индекс
             Sql.Clear;
             s:= 'CREATE TABLE trenddef (cod SERIAL, iname char(50), icomment TEXT, eu TEXT, mineu FLOAT, maxeu FLOAT, minraw FLOAT, maxraw FLOAT, logdb FLOAT,logtime FLOAT, deadbaund FLOAT, onmsg INTEGER, offmsg INTEGER , almsg INTEGER, logged INTEGER,  PRIMARY KEY(cod))';
             Sql.Add (s);
             ExecSQL;
             TrendConnection.Disconnect;
             TrendConnection.connect;
...
function TDataPostgree.WriteLog (iLogCode: longInt; Data: real; time: TDateTime; validLevel: boolean): integer;
var
   fn, s : string;
...
function TDataPostgree.WriteAlarm(id: integer; igroup, itag, iComment: string; iState: string; iAlarmLevel: integer; comp: string; oper: string; time:  TDateTime): boolean;
var
...
function TDataPostgree.WriteReport(Code:longInt;  typ: integer; Data: real; time: TDateTime): integer;
var
   fn, s : string;
  // per: integer;
begin
...
function TDataPostgree.WriteTrendef (iLogCode: longInt; iname: string; comment: string; MinEu, MaxEU, minRaw, maxRaw,logDB,logTime, DeadBaund: real; EU: string;
...
function TDataPostgree.delTable(fn: string): boolean;
var s: string;
begin
result:=true;
...
function TDataPostgree.GetLastRep (rtid: integer; typ_: integer; seachV: TdateTime; var tms: TdateTime ): boolean;
var
   fn, s : string;
begin
...
function TDataPostgree.GetLogID(iname: string): integer;
  var
   fn, s, iname_ : string;
   recordexist: boolean;
begin
...
procedure TDataPostgree.GetLogData(id: integer; starttime: TDateTime; stopTime: TDateTime; endp: boolean = true);
var i,j: integer;
    firsttable: string;
    fn, s: string;
    val: TDateTime;
...
procedure TDataPostgree.GetRepData(id: integer; typ_: integer; starttime: TDateTime; stopTime: TDateTime);
var i,j: integer;
    fn, s, h1, h2: string;
    val: TDateTime;
    isfind: boolean;
...
function TDataPostgree.CheckConnection(E: Exception):integer;
begin
    try
...
procedure TDataPostgree.FreeAndReLease;
begin
       try
       TrendQuery.FreeOnRelease;
       if TrendQuery.Active then
...
function TDataPostgree.getJournalDS(starttime: TDateTime; stopTime: TDateTime; msgset: TJournalMessageSet; tgList: TStrings; aggroups: TStrings; hosts: TStrings; operators: TStrings; srtAl: integer=-1; stpAl: integer=-1): TDataSet;
var i,j: integer;
    firsttable: string;
    fn, s: string;
    val: TDateTime;
...
function TDataPostgree.getReportTab(starttime: TDateTime; stopTime: TDateTime; RU: TReportUtit): TDataSet;
var i,j,k: integer;
...
procedure TDataPostgree.loadTrdef(val : TTrenddefList; error: boolean=false);
var fn,s: string;
begin
   fn:='trenddef';
   try
...
function TDataPostgree.getRepCountData_(itag: string; expr_: string; strlist: strarray;  intlist: intarray; typ_: integer; time: TDateTime; var ress: real): boolean;
   ...
   fn := ReportDateToFileName(time, typ_);
       if TableExist(fn) then
         begin
           ...
           if length(strlist)>0 then tmp_query:=tmp_query+' and '+ strlist[0]+'.tm ='+':tm';
