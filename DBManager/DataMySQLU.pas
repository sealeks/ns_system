...
type
  TDataMySQL = class(TMainDataModule)
...
TrendConnection.Protocol:='mysql';

end;


procedure TDataMySQL.uninit();
begin
...
function TDataMySQL.Connect(): boolean;
begin
  if TrendConnection.Connected then  TrendConnection.Disconnect;
  if  (trim(Schema)<>'') and (trim(DataBase)='') then
  TrendConnection.Database:=Schema else
...
function TDataMySQL.DisConnect(): boolean;
begin
  if TrendConnection.Connected then  TrendConnection.Disconnect;
  result:= not TrendConnection.Connected;
end;

function TDataMySQL.getConnected(): boolean;
begin
   result:= TrendConnection.Connected;
  
end;

function TDataMySQL.TableExist(iFileName: String): boolean;
var
...
function TDataMySQL.CreateTrTable(iFileName: String): boolean;
 var fn, s : string;
begin
  fn := iFileName;
  try
...
function TDataMySQL.CreateAlTable(iFileName: String): boolean;
 var fn, s : string;
begin
  fn := iFileName;
  try
...
function TDataMySQL.CreateRepTable(iFileName: String): boolean;
 var fn,s : string;
begin
  fn := iFileName;
  try
...
function TDataMySQL.createTrenddef(): boolean;
 var fn,s : string;
begin
     ...
     begin
       try
         with TrendQuery do
            ...
            // создать таблицу и индекс
             Sql.Clear;
             s:= 'CREATE TABLE trenddef (cod INTEGER NOT NULL AUTO_INCREMENT,';
             s:=s+' iname char(50), icomment TEXT, eu TEXT, mineu FLOAT, maxeu FLOAT, minraw FLOAT, maxraw FLOAT, logdb FLOAT,logtime FLOAT, deadbaund FLOAT, onmsg INTEGER, offmsg INTEGER, almsg INTEGER, logged INTEGER, PRIMARY KEY(cod))';
             Sql.Add (s);
             ExecSQL;
             TrendConnection.Disconnect;
             TrendConnection.connect;
...
function TDataMySQL.WriteLog (iLogCode: longInt; Data: real; time: TDateTime; validLevel: boolean): integer;
var
   fn, s : string;
...
function TDataMySQL.WriteAlarm(id: integer; igroup, itag, iComment, iState: string; iAlarmLevel: integer; comp: string; oper: string; time:  TDateTime): boolean;
var
...
function TDataMySQL.WriteReport(Code:longInt;  typ: integer; Data: real; time: TDateTime): integer;
var
   fn, s : string;
  // per: integer;
begin
...
function TDataMySQL.WriteTrendef (iLogCode: longInt; iname: string; comment: string; MinEu, MaxEU, minRaw, maxRaw,logDB,logTime, DeadBaund: real; EU: string;
...
function TDataMySQL.delTable(fn: string): boolean;
var s: string;
begin
result:=true;
...
function TDataMySQL.GetLastRep (rtid: integer; typ_: integer; seachV: TdateTime; var tms: TdateTime ): boolean;
var
   fn, s : string;
begin
...
function TDataMySQL.GetLogID(iname: string): integer;
  var
   fn, s, iname_ : string;
   recordexist: boolean;
begin
...
procedure TDataMySQL.GetLogData(id: integer; starttime: TDateTime; stopTime: TDateTime; endp: boolean = true);
var i,j: integer;
    firsttable: string;
    fn, s: string;
    val: TDateTime;
...
procedure TDataMySQL.GetRepData(id: integer; typ_: integer; starttime: TDateTime; stopTime: TDateTime);
var i,j: integer;
...
function TDataMySQL.CheckConnection(E: Exception):integer;
begin

     //TrendConnection.Commit;
    // TrendConnection.Disconnect;
 //   if  not TrendConnection.Connected then
   //   E:=LostConnectionError.Create('');
 //   raise E;
end;

procedure TDataMySQL.FreeAndReLease;
begin
       try
       TrendQuery.FreeOnRelease;
       if TrendQuery.Active then
...
function TDataMySQL.getJournalDS(starttime: TDateTime; stopTime: TDateTime; msgset: TJournalMessageSet; tgList: TStrings; aggroups: TStrings; hosts: TStrings; operators: TStrings; srtAl: integer=-1; stpAl: integer=-1): TDataSet;
var i,j: integer;
    firsttable: string;
    fn, s: string;
    val: TDateTime;
...
function TDataMySQL.getReportTab(starttime: TDateTime; stopTime: TDateTime; RU: TReportUtit): TDataSet;
var i,j,k: integer;
...
procedure TDataMySQL.loadTrdef(val : TTrenddefList; error: boolean=false);
var fn,s: string;
begin
   fn:='trenddef';
   try
...
function TDataMySQL.getRepCountData_(itag: string; expr_: string; strlist: strarray;  intlist: intarray; typ_: integer; time: TDateTime; var ress: real): boolean;
