...
type
  TForm1 = class(TForm)
    ...
    function AddRTitems(num: integer;
      Name, comment, ddeitem, onmsg, ofmsg, alarmmsg, eu: string; newitem: TAnalogMem;
      save: boolean; var replaceAll: boolean): boolean;
    ...
    procedure TreeViewProjectMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: integer);
  ...
  private
    ...
    procedure exportGroup(GroupName: string = '');
    //по умолчанию экспортировать все группы
    procedure LoadRegestry;
    procedure LoadMetadata(root: TTreeNode);
    procedure CreateMetadata;
    procedure SaveMetadata;
    procedure AddMetaNode(xmlParent: IXMLNode; parent: TTreenode;
      nm: string; header: boolean = True);
  ...
  public
    ...
    function DefDflt(var dfltBCD, dfltAlarmedLocal: boolean;
      ...
      dfltMinRaw, dfltMaxRaw, dfltAlConst:
      real; var dfltLogtime): TModalResult;
...
begin
  ...
  try
    ...
    begin
      MessageBox(0, PChar('Текущий проект принадлежит старой версии!  ' +
        char(13) + 'Произведите преобразование в менеджере поектов!'), 'Сообщение',
    ...
    MessageBox(0, PChar('Фатальная ошибка при открытии проекта!  ' +
      char(13) + 'Файлы проекта повреждены!'), 'Ошибка',
...
begin
  ...
  begin
    ...
    if (CheckVer(GetCurrentApp) = CONFFILE_V1) then
      curProj.ImageIndex := PRJ_MAINOLD
    else
  ...
  begin
    ...
    begin
      try
        if ((CheckVer(TStr.Strings[i]) = CONFFILE_V1) or
          (CheckVer(TStr.Strings[i]) = CONFFILE_V2)) then
        begin
          temp := TreeViewProject.Items.AddChild(someProj, TStr.Strings[i]);
          if CheckVer(TStr.Strings[i]) = CONFFILE_V1 then
            temp.ImageIndex := PRJ_SOMEOLD
          else
  ...
  begin
    ...
    begin
      ...
      if RightClickedItem <> nil then
        nt := TNSNodeType(RightClickedItem.ImageIndex)
      else
        nt := ntGG;
      case nt of
        ntO:
      ...
      begin
        ...
        if RightClickedItem <> nil then
          ImageIndex := RightClickedItem.ImageIndex +
            ifThen(TNSNodeType(RightClickedItem.ImageIndex) in
            NodeBaseSet, MainGroupCount, 0)
        else
            ...
            if TTreeNode(list.Items[i]).Parent <> nil then
              if TNSNodeType(TTreeNode(list.Items[i]).Parent.ImageIndex) = ntA then
                if rtItems.GetSimpleID(TTreeNode(list.Items[i]).Text) <> -1 then
                  rtItems[rtItems.GetSimpleID(TTreeNode(list.Items[i]).Text)].logged :=
                    False;
          ntGl:
          begin
...
begin
  ...
  try
    ...
    case TNSNodeType(dest.ImageIndex) of
      ...
      ntGl, ntG:
        for i := 0 to dragList.Count - 1 do
          ...
          begin
            rtitems.AlarmGroups.Items[rtitems.AlarmGroups.Idx[TTreeNode(
              dragList[i]).Text]].App := dest.Text;
            rtitems.AlarmGroups.SaveToFile(PathMem + 'alarmgroups.cfg');
          end
          else
            rtItems[rtItems.GetSimpleID(TTreeNode(dragList[i]).Text)].AlarmGroup :=
              rtitems.AlarmGroups.ItemNum[dest.Text];
      //бросаем в архив
      ntA:
        for i := 0 to dragList.Count - 1 do
        begin
      ...
      ntT:
        ...
        begin
          ...
          begin
            ...
            if rtItems.GetAlarmMsg(ID) <> '' then
              rtItems.SetAlarmMsg(ID, rtItems.GetComment(ID));
          end;
          dest.DeleteChildren;
    ...
    if (Sender is TTreeView) and (TNSNodeType(dest.ImageIndex) in ntOL) then
      if (TTreeNode(dragList[0]).Parent.Text = 'Аналоговые') then
        dest := dest.Item[0]
      else
      if TTreeNode(dragList[0]).Parent.Text = 'Дискретные' then
        dest := dest.Item[1];
    ...
    for i := 0 to dragList.Count - 1 do
      with(Sender as TTreeView).Items.AddChild(dest, TTreeNode(dragList[i]).Text) do
        if (TNSNodeType(dest.ImageIndex) = ntGG) or
          (TNSNodeType(dest.ImageIndex) = ntGGl) then
          ImageIndex := integer(ntGGl)
        else
          ImageIndex := TTreeNode(dragList[i]).ImageIndex;
    dest := saveDest;

    if Source = ListView1 then
    begin
      for i := ListView1.Items.Count - 1 downto 0 do
        if ListView1.Items[i].Selected and dragMove then
...
begin
  ...
  if (not ActionMove) and (SourceTypeSet <=
    acceptCopyArray[TNSNodeType(DestItem.ImageIndex)]) then
    cursor := crDragAdd;
  //а если хотим перемещать и разрешено переносить, то разрешено
  if (ActionMove) and (SourceTypeSet <=
    acceptMoveArray[TNSNodeType(DestItem.ImageIndex)]) then
    cursor := crDrag;
  //а если нельзя делать что хотим, делаем что можно
  if (cursor <> crDrag) and (cursor <> crDragAdd) then
    if (SourceTypeSet <= acceptCopyArray[TNSNodeType(DestItem.ImageIndex)]) then
      cursor := crDragAdd
    else if (SourceTypeSet <= acceptMoveArray[TNSNodeType(DestItem.ImageIndex)]) then
      cursor := crDrag;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        for i := 0 to rtItems.Count - 1 do
          if (rtItems[i].GroupNum = GroupNum) and (rtItems[i].ID <> -1) then
    ...
    case TNSNodeType(node.ImageIndex) of
      ...
      begin
        for i := 0 to rtItems.Count - 1 do
          if (rtItems[i].ID <> -1) and (rtItems[i].logTime in REPORT_SET) then
            with AddChild(node, rtItems.GetName(i)) do
    ...
    case TNSNodeType(node.ImageIndex) of
      ...
      begin
        ...
        begin
          ...
          NewNode.ImageIndex :=
            integer(gettype(rtitems.TegGroups.Items[i].App,
            rtitems.TegGroups.Items[i].Topic, rtitems.TegGroups.Items[i].Name));
          //integer(ntOl);
      ...
      begin
        ...
        for i := 0 to rtitems.Alarmgroups.Count - 1 do
          ...
          begin
            ...
            hasChildren := HasAlarmGroupItems(
              rtitems.AlarmGroups.ItemNum[rtitems.AlarmGroups.Items[i].Name]);
          end;
      ...
      begin
        for i := 0 to rtItems.Count - 1 do
          if (rtItems[i].AlarmGroup = rtitems.AlarmGroups.ItemNum[Node.Text]) and
            (rtItems[i].ID <> -1) then
            with AddChild(node, rtItems.GetName(i)) do
...
begin
  ...
  begin
    ...
    begin
      if ((TNSNodeType(TreeView1.Selections[k].ImageIndex) <>
        TNSNodeType(Node.ImageIndex)) and not
        ((TNSNodeType(TreeView1.Selections[k].ImageIndex) in ntArch) and
  ...
  begin
    ...
    itemcoord.setList(list, TNSNodeType(Node.ImageIndex), TNSNodeType(
      Node.Parent.ImageIndex));
    itemcoord.refresh;
    list.Free;
    exit;
  end;
...
begin
  ...
  begin
    ...
    PopupMenu1.Items[1].Visible :=
      (TNSNodeType(RightClickedItem.ImageIndex) in NodeCanDeletedSet) and
    ...
    begin
      if (TTreeNode(RightClickedItem).Parent <> nil) then
        PopupMenu1.Items[1].Visible :=
          (TNSNodeType(TTreeNode(RightClickedItem).Parent.ImageIndex) <> ntA);
    end;

    //PopupMenu1.Items[6].Visible:=false;
    // доделать
    PopupMenu1.Items[6].Visible :=
      ((TNSNodeType(RightClickedItem.ImageIndex) in ntOl) and not
    ...
    begin
      PopupMenu1.Items[7].Items[0].Visible :=
        (rtitems.TegGroups.ItemNum['$SYSVAR'] < 0);
      PopupMenu1.Items[7].Items[1].Visible :=
        (rtitems.TegGroups.ItemNum['$SYSTEM'] < 0);
      PopupMenu1.Items[7].Items[2].Visible :=
        (rtitems.TegGroups.ItemNum['$REPORT'] < 0);
      PopupMenu1.Items[7].Items[3].Visible :=
        (rtitems.TegGroups.ItemNum['$POINTER'] < 0);
      PopupMenu1.Items[7].Items[4].Visible :=
        (rtitems.TegGroups.ItemNum['$SERVERCOUNT'] < 0);
      PopupMenu1.Items[7].Items[5].Visible :=
        (rtitems.TegGroups.ItemNum['$COMMANDSCASE'] < 0);
    end;
    ...
    begin
      ...
      begin
        PopupMenu1.Items[18].Visible :=
          (RightClickedItem.Parent.IndexOf(RightClickedItem) <>
          (RightClickedItem.Parent.Count - 1));
...
begin
  ...
  for i := 0 to rtItems.Count - 1 do
    if (rtItems[i].ID <> -1) and rtItems[i].AlarmedLocal and
      (FromPriority <= rtItems[i].AlarmLevel) and
      (rtItems[i].AlarmLevel <= toPriority) then
      exit;
  Result := False;
end;
...
begin
  Result := True;
  for i := 0 to rtItems.Count - 1 do
    if (rtItems[i].ID <> -1) and (rtItems[i].GroupNum = GroupNum) and
      not rtItems.IsDiscret(i) then
      exit;
  Result := False;
end;
...
begin
  Result := True;
  for i := 0 to rtitems.AlarmGroups.Count - 1 do
    if rtitems.AlarmGroups.Items[i].App = rtitems.AlarmGroups[GroupNum] then
      exit;
  for i := 0 to rtItems.Count - 1 do
...
begin
  ...
  if (Source is TTreeView) then
    for i := 0 to (Source as TTreeView).SelectionCount - 1 do
      Result := Result + [TNSNodeType(
        (Source as TTreeView).Selections[i].ImageIndex)];
  if (Source is TListView) then
    for i := 0 to (Source as TListView).Items.Count - 1 do
      if (Source as TListView).Items[i].Selected then
        Result := Result + [TNSNodeType(
          (Source as TListView).Items[i].ImageIndex)];
end;
...
begin
  ...
  if (Source is TTreeView) then
    (Source as TTreeView).GetSelections(list);
...
begin
  ...
  if (Source = TreeView1) then
    parenTNSNodeType := TNSNodeType(TreeView1.Selected.Parent.ImageIndex)
  else
    parenTNSNodeType := TNSNodeType(ListedItem.ImageIndex);
  Result := not (parenTNSNodeType in DefaultCopyArray[TNSNodeType(DestItem.ImageIndex)]);
  //из массива
...
function cbSelectItem(cb: tComboBox; it: string): boolean;
  //выбирает в выпадающем списке совпадающую заись
var
  i: integer;
begin
  Result := False;
...
// определяем имя для нового тега
  ...
  begin
    ...
    begin
      if (rtitems.TegGroups.Count = 0) then
        exit
      else
        Result := rtitems.TegGroups.Items[0].Num;
    end
    else
    begin
      ...
      begin
        ...
        begin
          ...
          if (tgid < 0) then
            ...
            Result := rtitems[tgid].GroupNum;
        end;
        if (TNSNodeType(TreeView1.Selections[0].ImageIndex) in ntOl) then
        begin
  ...
  begin
    ...
    for i := 0 to TreeView1.Items.Count - 1 do
      if ((TreeView1.Items[i].Text = groupname) and
        (TNSNodeType(TreeView1.Items[i].ImageIndex) in ntOl)) then
      begin
...
begin
  if trim(typeMod) = 'V' then
    Result := '0';
  if trim(typeMod) = 'B' then
    Result := '0';
  if trim(typeMod) = 'GX' then
    Result := '40000';
  if trim(typeMod) = 'GY' then
    Result := '40200';
  if trim(typeMod) = 'S' then
    Result := '41000';
  if trim(typeMod) = 'T' then
    Result := '41100';
  if trim(typeMod) = 'SP' then
    Result := '41200';
  if trim(typeMod) = 'X' then
    Result := '40400';
  if trim(typeMod) = 'Y' then
    Result := '40500';
  if trim(typeMod) = 'C' then
    Result := '40600';
end;
  ...
  try
    ...
    begin
      if rtitems.TegGroups.itemNum[GroupName] <> -1 then
        raise Exception.Create('Группа уже существует.');
      group.Num := -1;
      //Когда будем добавлять в список, будет заменен на первый свободный номер группы
...
begin
  ...
  for i := 0 to rtItems.Count - 1 do
    if rtItems[i].ID <> -1 then
      if (less = -1) and (UpperCase(rtItems.GetName(i)) < curName) then
        less := i
      else if (less <> -1) and (UpperCase(rtItems.GetName(i)) < curName) and
        (UpperCase(rtItems.GetName(less)) < UpperCase(rtItems.GetName(i))) then
        less := i;
  if less <> -1 then
  begin
    curIt := less;
...
begin
  ...
  for i := 0 to rtItems.Count - 1 do
    if rtItems[i].ID <> -1 then
      if (more = -1) and (UpperCase(rtItems.GetName(i)) > curName) then
        more := i
      else if (more <> -1) and (UpperCase(rtItems.GetName(i)) > curName) and
        (UpperCase(rtItems.GetName(more)) > UpperCase(rtItems.GetName(i))) then
        more := i;
  if more <> -1 then
  begin
    curIt := more;
...
begin
  ...
  for i := 1 to rtItems.Count - 1 do
    if rtItems[i].ID <> -1 then
      if UpperCase(rtItems.GetName(i)) > UpperCase(rtItems.GetName(Result)) then
        Result := i;
end;
...
begin
  ...
  if TreeView1.GetNodeAt(pnt.X, pnt.Y) <> nil then
    StatusBar1.Panels[0].Text :=
      TreeView1.GetNodeAt(pnt.X, pnt.Y).Text + ':' +
...
begin
  ...
  nextItem := CurItem;
  if not FindNextMore(NextItem) then
  ...
  for i := TreeView1.Items.Count - 1 downto 0 do
    if TreeView1.Items[i].Text = TegName then
...
begin
  for i := 0 to TreeView1.Items.Count - 1 do
    if ((TreeView1.Items[i].Text = nodeName) and
      (TNSNodeType(TreeView1.Items[i].ImageIndex) in ntype)) then
    begin
      refreshNode(TreeView1.Items[i], True);
      exit;
    end;
...
begin
  ExportGroup(AnsiUpperCase(inputBox('Ввод пользователя',
    'Введите экспортируемую группу:', '')));
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (trim(StringGrid1.Cells[0, 1]) <> 'B') and
          (trim(StringGrid1.Cells[0, 1]) <> 'V') then
        begin
          ...
          if not InputQuery('Начальный адрес', 'Импортировать в V:', ImportTo) then
            exit;
        ...
        begin
          ...
          if isNew and not KnowDflt then
            if DefDflt(dfltBCD, dfltAlarmedLocal, dfltGroup,
              dfltAlCase, dfltAlLevel, dfltMinEu,
              dfltMaxEu, dfltMinRaw, dfltMaxRaw,
              dfltAlConst, dfltLogTime) <> mrOk then
              exit
            else
              KnowDflt := True;
          ...
          _ddeitem := 'V' + ShiftAddr(StringGrid1.Cells[0, i],
            StringGrid1.Cells[1, i], shiftWord);
          ...
          if isNew then
            ...
            begin
              ...
              newitem.logDB :=
                strtofloat_(format('%g', [(dfltMaxEu - dfltMinEu) / 200])); //0.5%
              // end;
          ...
          if AddRTitems(i, rtName, _comment, _ddeitem, _onmsg,
            _ofmsg, _alarmmsg, _eu, newitem, True, replaceAll) then
            exit;
        end;
  ...
  with TFrmEditCSV.Create(self) do
    try
      ...
      for i := 0 to StringGrid1.ColCount - 1 do
        if UpperCase(StringGrid1.Cells[i, 0]) = 'NAME' then
          NameCol := i
        else
        if (UpperCase(StringGrid1.Cells[i, 0]) <> 'GROUP') and
          ...
          (UpperCase(StringGrid1.Cells[i, 0]) <> 'DEADBAUND') then
          raise Exception.Create('Заголовок не определен "' +
            StringGrid1.Cells[i, 0] + '"');
      if NameCol = -1 then
        raise Exception.Create('Поле NAME не найдено');

      progressBar1.Max := StringGrid1.RowCount - 1;
      for j := 1 to StringGrid1.RowCount - 1 do
      begin
        progressBar1.position := j;
        if StringGrid1.Cells[NameCol, j] = '' then
          continue;
        rtName := pref + StringGrid1.Cells[nameCol, j];
        StatusBar1.Panels[0].Text :=
          IntToStr(j) + '/' + IntToStr(StringGrid1.RowCount) + ':' + rtName;
        StatusBar1.Refresh;
        progressBar1.refresh;
        ...
        for i := 0 to StringGrid1.ColCount - 1 do
          ...
          begin
            if trim(group_) = '' then
              ...
              newitem.GroupNum := rtitems.TegGroups.ItemNum[trim(group_)];

          end
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ITEM') then
            _ddeitem := StringGrid1.Cells[i, j]
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'COMMENT') then
            _comment := StringGrid1.Cells[i, j]
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'LOGGED') then
            newitem.Logged := (UpperCase(StringGrid1.Cells[i, j]) = 'TRUE')
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'LOGTIME') then
            newitem.logTime := StrToInt(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'LOGDB') then
            newitem.logDB := strtofloat_(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'MINRAW') then
            newitem.MinRaw := strtofloat_(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'MAXRAW') then
            newitem.MaxRaw := strtofloat_(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'MINEU') then
            newitem.Mineu := strtofloat_(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'MAXEU') then
            newitem.Maxeu := strtofloat_(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ALARMEDLOCAL') then
            newitem.AlarmedLocal := (UpperCase(StringGrid1.Cells[i, j]) = 'TRUE')
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ALARMMSG') then
            _alarmmsg := StringGrid1.Cells[i, j]
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'EU') then
            _eu := StringGrid1.Cells[i, j]
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ALARMLEVEL') then
            newitem.Alarmlevel := StrToInt(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ALARMCASE') then
            newitem.alarmCase := TAlarmCase(trim(StringGrid1.Cells[i, j]) = '<')
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'DEADBAUND') then
            DeadBaund := strtofloat_(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ALARMCONST') then
            newitem.AlarmConst := strtofloat_(StringGrid1.Cells[i, j])
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ALARMGROUP') then
            newitem.AlarmGroup :=
              rtitems.alarmgroups.ItemNum[trim(StringGrid1.Cells[i, j])]
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ONMSGED') then
            newitem.OnMsged := (UpperCase(StringGrid1.Cells[i, j]) = 'TRUE')
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'ONMSG') then
            _onmsg := StringGrid1.Cells[i, j]
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'OFFMSGED') then
            newitem.OffMsged := (UpperCase(StringGrid1.Cells[i, j]) = 'TRUE')
          else
          if (UpperCase(StringGrid1.Cells[i, 0]) = 'OFFMSG') then
            _ofmsg := StringGrid1.Cells[i, j];
        if AddRTitems(i, rtName, _comment, _ddeitem, _onmsg, _ofmsg, _alarmmsg,
          _eu, newitem, True, replaceAll) then
          exit;
      end;
      RefreshNodeName('Опрос');
    finally
...
begin
  if SaveDialog1.Execute then
    if not FileExists(SaveDialog1.FileName) or
      ...
      (MessageDlg('Файл ' + ExtractFileName(SaveDialog1.FileName) +
      ' существует. Перезаписать?', mtConfirmation, [mbYes, mbNo], 0) = idYes))) then
      try
...
begin
  ...
  begin
    if (NodeType = TegType) or (rtItems.GetSimpleID(node.Text) = -1) then
      ...
      node.ImageIndex := TImageIndex(TegType);
...
begin
  ...
  if ((TNSNodeType(TreeView1.Selected.ImageIndex) in ntOl) and
    (changeGrMI <> nil)) then
  begin
...
begin
  ...
  for i := 0 to TreeView1.Items.Count - 1 do
    if ((TreeView1.Items[i].Text = 'Опрос') and
      (TNSNodeType(TreeView1.Items[i].ImageIndex) = ntO)) then
    begin
...
begin
  ...
  if (rtitems.TegGroups.Count < 1) then
    exit;
  if (TreeView1.SelectionCount = 0) then
    exit;
  ...
  begin
    ...
    begin
      ...
      tempmenu.ImageIndex :=
        integer(gettype(rtitems.TegGroups.Items[i].App, rtitems.TegGroups.Items[i].Topic,
        rtitems.TegGroups.Items[i].Name));
...
begin
  ...
  if (rtitems.Alarmgroups.Count < 1) then
    exit;
  if (TreeView1.SelectionCount = 0) then
    exit;
  tmpstr := TreeView1.Selections[0].Text;
...
begin
  ...
  if num > -1 then
    refreshNodeNameandtype(rtitems.Alarmgroups.Items[TMenuItem(Sender).Tag].Name,
      [ntGT, ntT]);
  TreeAlTagOutofAlarmGroup;
  list.Free;
end;

function TForm1.AddRTitems(num: integer;
  Name, comment, ddeitem, onmsg, ofmsg, alarmmsg, eu: string; newitem: TAnalogMem;
  save: boolean; var replaceAll: boolean): boolean;
var
  i: integer;
begin
  ...
  begin
    MessageDlg('Тег в строке N' + IntToStr(num) + ' имеет некорректное имя (' + Name +
      ') и не будет добавлен. ',
      mtConfirmation, [mbYes], 0);
    exit;
  end;
...
var
  ...
  fl: boolean;
begin
  if MessageDlg('Сжать базу???', mtWarning, [mbYes, mbNo], 0) <> mrYes then
    exit;
  fl := True;
  while (fl) do
  begin
...
begin
  if (TreeView1.SelectionCount = 1) and
    (TNSNodeType(TreeView1.Selected.ImageIndex) in tegset) then
  begin
...
begin
  if MessageDlg('Удалить всю базу???', mtWarning, [mbYes, mbNo], 0) <> mrYes then
    exit;
  rtItems.Count := 0;
...
procedure TForm1.TreeViewProjectMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: integer);
begin
  if button = mbRight then
  begin
...
begin
  ...
  begin
    if (TMenuItem(Sender).ImageIndex <> TreeView1.Selected.ImageIndex) then
      ...
      ValueListItem.Color := clWhite;

    ValueListItem.Enabled := ((TMenuItem(Sender).ImageIndex) =
...
begin
  ...
  begin
    ...
    if (TreeView1.Selected <> nil) and
      (TNSNodeType(TreeView1.Selected.ImageIndex) = ntO) then
    ...
    begin
      ...
      begin
        ...
        if (TMenuItem(Sender).ImageIndex <> integer(ntOBase)) then
          trnNew.ImageIndex := integer(ntOBase)
        else
...
procedure TForm1.AddMetaNode(xmlParent: IXMLNode; parent: TTreenode;
  nm: string; header: boolean = True);
var
  i: integer;
  str: string;
...
begin
  ...
  if (tmptype = cdtMainMessageHeader) then
    exit;
  if header then
  begin
  ...
  if (tmptype = cdtmeta) then
    tmpTreenode.ImageIndex := integer(cdtmeta);
    ...
    begin
      case gettypeMetaByname(tmpXML.LocalName) of
        ...
        begin
          ...
          begin
            if (Sender = N14) then
              newXML := tmpXML.AddChild('TrendHeader')
            else
  ...
  begin
    ...
    begin
      ...
      for i := 0 to tmpParTN.Count - 1 do
        if tmpParTN.Item[i].Text = str then
          TreeView1.Select(tmpParTN.Item[i]);
    end;
    if (Sender = NUp) then
    begin
      tmpParXML.ChildNodes.Remove(tmpXML);
        ...
        begin
          try
            ...
            try
              ...
              if TRENDLIST_GLOBAL_TRDF.Count = 0 then
                raise Exception.Create('Trenddef пуст или не существует');
            except
...
begin
  ...
  begin
    ...
    begin
      ...
      tmpXML.Attributes['group'] :=
        rtitems.TegGroups.ItemNameByNum[rtitems.Items[rtitems.Items[i].ID].GroupNum];
