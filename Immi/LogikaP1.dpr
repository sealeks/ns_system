...
//  FormConnect in  '..\Units\FormConnect.pas',
...
begin
  ...
  fTopMenu.Show;
  FLoad.ProcessLog('Запуск сервера');
  if FindWindow(Pchar(uppercase('TFormEtherServer')),pchar('Сервер данных Logika'))=0 then begin
     ZeroMemory(cif,sizeof(_STARTUPINFOA));
     CreateProcess(Pchar(PathEthSer),Pchar(''),
		   nil,nil,false,CREATE_SEPARATE_WOW_VDM,nil,nil,cif^,pip); end;
  if FindWindow(Pchar(uppercase('TFormComServer')),Pchar('Com Сервер данных'))=0 then begin
      ZeroMemory(cif,sizeof(_STARTUPINFOA));
      CreateProcess(Pchar(PathSer),Pchar(''),
	    	nil,nil,false,CREATE_SEPARATE_WOW_VDM,nil,nil,cif^,pip);end;
